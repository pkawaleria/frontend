...
export default function Profile() {
    ...
    return (
        <div className="flex flex-col items-center p-5 gradient-bg-color-only h-screen mt-10">
            <nav className="flex flex-col justify-center w-[50%] bg-blue-500 dark:bg-neutral-600 text-white p-4 rounded-t-lg mb-4">
            ...
            <div className="w-[60%] max-w-screen-md bg-white dark:bg-neutral-500 rounded-lg shadow-xl p-6 relative">
                ...
                <Link className="nav-link absolute bottom-1 right-1 bg-red-500 dark:bg-red-900 dark:hover:bg-red-800 text-white py-3 pr-3 pl-2 rounded-full hover:bg-red-800 easy-linear duration-200 focus:outline-none" to="/wyloguj" data-tooltip-id='logoutTooltip' data-tooltip-content="Wyloguj się">
                    <BiLogOut size={20} />
                    <Tooltip id="logoutTooltip" type="dark" effect="solid" delayShow={50} delayHide={100} />
                </Link>
                <Link className="nav-link absolute bottom-1 right-12 bg-yellow-500 dark:bg-yellow-700 dark:hover:bg-yellow-600 text-white py-[10px] px-[10px] rounded-full hover:bg-yellow-600 easy-linear duration-200 focus:outline-none" to="/zmien-haslo">
                    Zmień hasło
                </Link>
            </div>
        </div>
