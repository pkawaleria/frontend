...
export default function GenericAuctionsListPage({ fetchFunction, pageNumber, pageSize, onPageChange }) {
    ...
    const handleNextPage = () => {
        if (pageNumber < pagedAuctions.pageCount) {
    ...
    return (
        <div className="h-[70%] py-6 px-6 flex flex-col items-center justify-center bg-linear-top-bottom">
            ...
            <div className="flex">
                <BiChevronsLeft
                    className={`m-auto icon-hover text-white text-[2.5vw] ease-linear duration-200 cursor-pointer ${
                        pageNumber === 1 ? 'disabled' : ''
                    }`}
                    onClick={handlePreviousPage}
                    disabled={pageNumber === 1} />
                    <span className="my-auto text-white">
                        Strona {pagedAuctions.pageNumber} z {pagedAuctions.pageCount}
                    </span>
                <BiChevronsRight
                    className={`m-auto icon-hover text-white text-[2.5vw] ease-linear duration-200 cursor-pointer ${
                        pageNumber === pagedAuctions.pageCount ? 'disabled' : ''
                    }`}
